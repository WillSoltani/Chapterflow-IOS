// Render-guard snapshot tests for the Social profile screen.
//
// Deterministic & hermetic: renders the real screen off-screen with
// `ImageRenderer` (fed by `FakeSocialRepository` — no network, no live clock)
...
@MainActor
@Suite("Social — render guards")
struct SocialRenderGuardTests {

    @Test("ProfileView renders across the matrix (loaded)")
    func loadedMatrix() {
        assertMatrix("Profile (loaded)") {
            ProfileView(repository: FakeSocialRepository.loaded)
        }
    }

    @Test("ProfileView renders its error state")
    func erroredMatrix() {
        assertMatrix("Profile (error)") {
