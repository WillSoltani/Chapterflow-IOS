...
/// `TokenProviding`-compatible actor for reading the cached token and
/// triggering a refresh when near-expiry.
///
/// Used in tests to verify token-reading logic independently of the full
/// `SessionManager`. In production the `APIClient` uses `SessionManager`
/// directly (which implements the full `TokenProviding` protocol including
/// step-up reauth).
///
/// Decision logic:
/// 1. No tokens in store → return `nil` (unauthenticated).
/// 2. Tokens are near-expiry (< 5 min) or expired → force-refresh, return new id_token.
