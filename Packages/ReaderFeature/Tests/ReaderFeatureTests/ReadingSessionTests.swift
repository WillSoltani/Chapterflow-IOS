...
struct ReadingSessionTests {
    ...
    func sessionStartedAfterLoad() async {
        let fake = FakeReaderRepository()
        let model = makeModel(repo: fake)
        model.load()
        await waitUntil(timeout: .seconds(5)) { !fake.startSessionCalls.isEmpty }
    ...
    func sessionIdPassedToHeartbeat() async {
        let fake = FakeReaderRepository()
    ...
    func sessionNotStartedOnError() async {
        let fake = FakeReaderRepository(
        ...
        await waitUntil(timeout: .seconds(5)) {
            if case .loading = model.phase { return false }
            return true
        }
    ...
    func sessionEndedOnDisappear() async {
        let fake = FakeReaderRepository()
        let model = makeModel(repo: fake)
        model.load()
        ...
        model.onDisappear()
        await waitUntil(timeout: .seconds(5)) { !fake.endSessionCalls.isEmpty }
    ...
    func sessionEndNotCalledWithoutLoad() async {
        let fake = FakeReaderRepository()
        let model = makeModel(repo: fake)
