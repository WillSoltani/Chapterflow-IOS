...
You are the sole implementation owner for **WP-ENGAGE-01 — Make engagement surfaces adaptive, owner-safe, and exactly routed**. Complete this one package through its guarded development Git lifecycle, or stop with precise evidence. Do not implement another package.
...
1. Adapt dashboard, daily-goal, and chart layouts to compact/iPad, AX text, keyboard/pointer, real-locale, pseudo-long, and RTL states.
2. Preserve valid independent sections and distinguish cached/unknown state from fresh data.
3. Give charts/heatmaps concise localized summaries and navigable values where action is required.
4. Route visible journey/event actions exactly or show truthful unavailability.
5. Keep progress, goal, reward, tier, and badge values server-authoritative and render unknown without a client grant.
6. Localize package-owned visible and accessibility copy, then clear owner-scoped dashboard, reward,
   route, and shared-snapshot state across account A → sign out → account B.
7. Publish only WP-EXT-01's versioned owner-bound snapshot; unknown ownership publishes nothing.
8. Implement and test truthful loading/cached/empty/error/offline/cancellation/auth/background/relaunch
   transitions without erasing valid sections or allowing stale work to commit.
